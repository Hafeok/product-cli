...
{ echo "gate A instrument record — $(date -u +%FT%TZ)"; echo "commit: $(git rev-parse HEAD) (working tree: $TREE uncommitted paths, captured before this record was created)"; echo "binary: target/release/product sha256 $(sha256sum $P | cut -c1-16)"; echo "inventory: A $(jq -r .inventory_version $A) sha256 $(sha256sum $A | cut -c1-16) (the run-8 artefact; reader not re-run)"; echo "ground truth: $(sha256sum $GT | cut -c1-16)"; } > $M/gateA-instrument.txt
...
$P csharp reach $A --roots "$AMAIN,$AFW" --track 'implements:T:Mediator.IRequestHandler`2' --track 'implements:T:Mediator.INotificationHandler`1' --ground-truth $RUN8/ground-truth-A-web.yaml > $M/gateA-regression-run8-A-primary.txt
if diff -q $M/gateA-regression-run8-A-primary.txt $RUN8/gate1a-measurement/run8-A-reach-PRIMARY-main-plus-framework-handlers.txt > /dev/null; then
  echo "regression: run-8 A primary reach output byte-identical" >> $M/gateA-instrument.txt
else
  echo "regression: run-8 A primary reach output DIFFERS" >> $M/gateA-instrument.txt; exit 1
fi
$P csharp candidates $A --ground-truth $GT > $M/gateA-A-candidates.txt
$P --format json csharp candidates $A --ground-truth $GT > $M/gateA-A-candidates.json
