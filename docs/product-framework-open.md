...
*A conformant instantiation of the Two Pillars Specification Framework. Version 1.4.0 — open specification.*
...
Everything above describes a system's *interior* — its domain, behaviour, screens, navigation — but until now nothing names *the system itself*. The **system** is a first-class What-side node: the named, identified thing the page graph and flows belong to. It carries:
...
A system **owns its page graph** (its root, §3.2.4) and **its flows**, but **not the domain model** — and that distinction is the point of allowing more than one system. A What may describe **several systems that share one domain model**: a customer-facing app and an internal admin website both speak the same `Order` and `Cart` (the shared domain, §3.1), but each is a *distinct surface* over that meaning, with its own root, its own flows, its own target platforms, and screens the other does not have. A flow belongs to exactly one system; the domain it reads is shared. This is the bounded-context story (§3.1) working as intended — the domain is the shared meaning, and each system is a different reach into it.

> **System identity is What; deployment identity is How.** The system node carries what the product *is and reaches* — its name, kind, purpose, target platforms. It does **not** carry its concrete address: the production domain name (`shop.acme.com`), the App Store bundle identifier, the chosen runtime. Those are realisation, and they live in the **infrastructure/runtime contract** (§4.2) — they vary per deployment (staging and production differ), they are frozen once chosen, which is exactly that contract's remit. The store-facing *display name* may itself be content (§4.6), resolved and localized. The What says "Acme Shop is a consumer e-commerce application targeting iOS, Android, and web"; the How says "iOS bundle `com.acme.shop`, web served at `shop.acme.com` from these runtimes." Meaning and reach above the line; concrete address below it.
...
|---|---|---|
| **What specification** | System(s) (name, kind, purpose, target platforms) + domain model (structure + reference data) + event model (behaviour, incl. UI steps typed by Abstract Interaction Objects + context-of-use + WCAG 2.2 accessibility criteria) + Decider (executable behaviour) + Projector (executable read model) + named-algorithm primitives (the Polanyi floor) | [§3](#3-the-what--structure-and-behaviour) |
...
2. The What has two halves: a domain model (structure) and an event model (behaviour), authored and agreed before the How. The What also names one or more **systems** (§3.2.5) — each a first-class node with a name, kind (app/website/service/…), purpose, and **target platform contexts** (platform being a context-of-use dimension, §3.2.2) and **target interaction classes** (GUI/TUI, the gating context dimension) — which own their page graphs and flows but share the domain model; a What may describe several systems (e.g. a customer app and an admin web) over one domain. System identity is What; concrete deployment identity (domain name, bundle id, runtime) is How (§4.2).
