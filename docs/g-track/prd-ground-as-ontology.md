...
- **Over-advertising.** kotlin-lsp declares `typeHierarchyProvider: true` and returns `null`.
  *(Second case, measured 2026-08-20 on a live full-solution run:)* **Roslyn advertises
  `workspaceSymbolProvider: true` and then does not answer `workspace/symbol`** on a 16-project
  solution, while answering it on a three-project subset of the same corpus. This adds a nuance the
  Kotlin case does not carry: **capability is not a property of the server alone but of server ×
  workspace size**, so the probe must run against the workspace actually being read, not against a
  toy one. It is also the dangerous direction demonstrated by a server this track depends on.
