...
> Generated by product v0.1 | Last updated: 2026-06-19
...
- [?] TC-715: failing_tc_remains_visible_to_graph_check_after_verify (scenario) — unrunnable
...
### FT-060 — Alphabetically Sorted CLI Help Output [x]
...
- [x] TC-725: top level subcommands listed alphabetically (scenario) — passing
- [x] TC-726: nested subcommand groups listed alphabetically (scenario) — passing
- [x] TC-727: cli subcommands are sorted fitness test catches regressions (invariant) — passing
- [x] TC-728: help output sortedness contract holds across full surface (exit-criteria) — passing

### FT-067 — Platform-scoped ADRs: separate enforced-by-platform decisions from per-feature gaps [x]
...
- [x] ADR-043: Slice + Adapter Architecture for Command Handlers (accepted)
- [x] TC-789: scope_platform_parses_and_round_trips (scenario) — passing
- [x] TC-790: adr_scope_platform_writes_field (scenario) — passing
- [x] TC-791: adr_list_scope_platform_filter (scenario) — passing
- [x] TC-792: preflight_platform_invariant_is_informational (scenario) — passing
- [x] TC-793: preflight_cross_cutting_still_gates (scenario) — passing
- [x] TC-794: gap_check_platform_no_enforcement (scenario) — passing
- [x] TC-795: gap_check_g010_cleared_when_tc_linked (scenario) — passing
- [x] TC-796: adr_scope_audit_dry_run_then_apply (scenario) — passing
...
- [?] TC-712: update_tc_status_escapes_double_quotes_in_failure_message (scenario) — unrunnable
- [?] TC-713: update_tc_status_escapes_newlines_in_failure_message (scenario) — unrunnable
- [?] TC-714: update_tc_status_escapes_backslashes_in_failure_message (scenario) — unrunnable
- [?] TC-715: failing_tc_remains_visible_to_graph_check_after_verify (scenario) — unrunnable
...
- [x] TC-704: product_schema_feature_documents_body_convention (scenario) — passing
...
### FT-061 — MCP Server and CLI Honor `.product/config.toml` Discovery [x]
...
- [x] TC-768: default-target-fallback-uses-human-template (scenario) — passing
- [x] TC-769: mcp-product-context-uses-id-parameter (scenario) — passing

### FT-064 — Strict Change-Spec Validation and Artifact Deletion Surface [x]

- [x] ADR-018: Testing Strategy — Property-Based, Session-Based, and LLM Benchmark (accepted)
- [x] ADR-043: Slice + Adapter Architecture for Command Handlers (accepted)
- [x] ADR-047: Functional Specification Lives in Feature Body, Not a Separate Artifact (accepted)
- [x] TC-770: request rejects unknown keys on a change block (scenario) — passing
- [x] TC-771: request rejects change with empty mutations list (scenario) — passing
- [x] TC-772: request rejects unknown keys on a mutation block (scenario) — passing
- [x] TC-773: request op:remove on list-valued field removes the entry (scenario) — passing
...
### FT-066 — MCP Parity for Feature/TC Status Writes and Reciprocal Linking [x]

- [x] ADR-010: Auto-Orphan Test Criteria on Feature Abandonment (accepted)
- [x] ADR-020: MCP Server — Dual Transport (stdio and HTTP) (accepted)
- [x] ADR-032: Content Hash Immutability Enforcement (accepted)
- [x] ADR-047: Functional Specification Lives in Feature Body, Not a Separate Artifact (accepted)
- [x] TC-778: mcp_feature_status_writes_to_disk (scenario) — passing
- [x] TC-779: mcp_test_status_writes_to_disk (scenario) — passing
- [x] TC-780: mcp_feature_status_abandonment_runs_orphan_cascade (scenario) — passing
- [x] TC-781: mcp_feature_status_in_progress_blocked_by_tc_runner_gate (scenario) — passing
- [x] TC-782: mcp_status_update_unknown_id_returns_error (scenario) — passing
- [x] TC-783: mcp_status_update_invalid_status_string_returns_error (scenario) — passing
- [x] TC-784: mcp_status_update_legacy_note_string_absent_from_codebase (invariant) — passing
- [x] TC-785: mcp_feature_link_reciprocates_tc_validates_features (scenario) — passing
- [x] TC-786: mcp_feature_link_reciprocates_adr_features (scenario) — passing
...
### FT-104 — Default-acknowledged cross-cutting ADRs with per-feature opt-out [x]
...
- [x] ADR-047: Functional Specification Lives in Feature Body, Not a Separate Artifact (accepted)
- [x] ADR-051: TC Observability Requirement — Causation Over Transaction (accepted)
- [x] TC-860: default_acknowledge_clears_preflight_gap (scenario) — passing
- [x] TC-861: adrs_rejected_reintroduces_gap_as_intentional (scenario) — passing
- [x] TC-862: default_ack_drift_emits_warnings (scenario) — passing
- [x] TC-863: ft_104_exit_criteria (exit-criteria) — passing

### FT-106 — Distribute Product LLM Skill via agent-init [ ]
...
- [x] ADR-047: Functional Specification Lives in Feature Body, Not a Separate Artifact (accepted)
- [x] ADR-050: Introduce Pattern (PAT) Artifact Type for Implementation Knowledge (accepted)
- [ ] TC-876: agent-init writes skill to project path (scenario) — unimplemented
- [ ] TC-877: agent-init respects install false config (scenario) — unimplemented
- [ ] TC-878: agent-init overlay replaces embedded skill (scenario) — unimplemented
- [ ] TC-879: agent-init skill target user expands home (scenario) — unimplemented
- [ ] TC-880: skill install is idempotent (invariant) — unimplemented
- [ ] TC-881: installed skill has valid frontmatter (exit-criteria) — unimplemented
- [ ] TC-882: agent-init writes copilot SKILL.md to .github/skills when target enabled (scenario) — unimplemented
- [ ] TC-883: all enabled skill targets are byte-identical (invariant) — unimplemented
- [ ] TC-884: targets list controls which paths receive the skill (scenario) — unimplemented

### FT-107 — Split product-cli into product-core, product-mcp, product-cli workspace [x]
...
- [x] ADR-043: Slice + Adapter Architecture for Command Handlers (accepted)
- [x] ADR-048: Canonical Repository Layout — All Product State Under `.product/` (accepted)
- [x] TC-885: product-core compiles standalone without clap or axum (scenario) — passing
- [x] TC-886: product-mcp depends on product-core and exposes ToolRegistry (scenario) — passing
- [x] TC-887: product-cli binary builds and product --help still works (scenario) — passing
- [x] TC-888: cargo t passes across the workspace with split crates (exit-criteria) — passing
- [x] TC-889: external crate can depend on product-core alone (scenario) — passing
- [x] TC-890: code-quality 400-line gate runs against every workspace member (invariant) — passing

### FT-108 — Two Pillars conformance check over the knowledge graph [x]

- [x] ADR-052: Two Pillars clauses are checked structurally from the knowledge graph (accepted)
- [x] TC-891: conformance check passes on conforming repo (scenario) — passing
- [x] TC-892: conformance check flags incomplete what specification (scenario) — passing
- [x] TC-893: conformance check flags undeclared product decision (scenario) — passing
- [x] TC-894: conformance check flags unjudged complete feature (scenario) — passing
- [x] TC-895: conformance check json reports clauses and profile (scenario) — passing

### FT-109 — product author domain — facilitated What-capture MCP session [x]

- [x] ADR-053: Domain authoring is a separate What graph with native in-loop conformance (accepted)
- [x] TC-896: author domain print-prompt emits facilitation prompt (scenario) — passing
...
### FT-115 — product how add/set — granular authoring of the Why cascade and contracts [x]

- [x] ADR-057: How elements are authored granularly via add/set on the contract file (accepted)
- [x] TC-950: how build full contract from scratch (scenario) — passing
- [x] TC-951: how add duplicate id is rejected (scenario) — passing
- [x] TC-952: how add resource requires infra contract (scenario) — passing
...
- [x] ADR-087: The domain (What) graph is exposed as product_domain_* MCP tools (accepted)
...
### FT-120 — product archetype check — enforce a layout model against the repository tree [x]

- [x] ADR-086: Layout conformance applies glob rules to the filesystem with allowlist semantics (accepted)
- [x] TC-945: archetype check layout against the tree (scenario) — passing

### FT-121 — product decider — derive an aggregate's executable signature and validate drift [x]

- [x] ADR-061: A Decider's signature is derived from and validated against the event model (accepted)
- [x] TC-946: decider derive and validate (scenario) — passing
- [x] TC-947: decider foreign command nonconformant (scenario) — passing

### FT-122 — product decider simulate — prove a Decider sound and complete before realisation [x]

- [x] ADR-062: Decider logic is a declarative guarded state machine simulated as pure functions (accepted)
- [x] ADR-063: CEL is the Decider expression language for guards and assignments (accepted)
- [x] TC-948: decider simulate sound and complete (scenario) — passing
- [x] TC-949: decider simulate catches wrong behaviour (scenario) — passing
- [x] TC-955: decider simulate cel guard and payloads (scenario) — passing

### FT-123 — product decider conform — check realised code against the Decider oracle [x]

- [x] ADR-064: Behavioural conformance replays the Decider scenarios against a pluggable runner (accepted)
- [x] TC-956: decider conform matching runner passes (scenario) — passing
- [x] TC-957: decider conform divergent runner fails (scenario) — passing

### FT-124 — product slice — a saved pointer to a section of the event model [x]

- [x] ADR-065: A delivery slice is a pointer into the event model; its context is derived from the graph (accepted)
- [x] TC-958: slice new and context assemble the subgraph (scenario) — passing
- [x] TC-959: slice new rejects a dangling anchor (scenario) — passing

### FT-125 — product status surfaces the framework What, How, and delivery graph [x]

- [x] ADR-066: product status summarises the framework graph alongside the legacy spec (accepted)
- [x] TC-966: status shows the framework graph (scenario) — passing

### FT-126 — product deliverable and release — the delivery layer over slices [x]

- [x] ADR-067: Delivery units are id-pointers — release to deliverable to slice (accepted)
- [x] TC-967: delivery chain release feature slice (scenario) — passing
- [x] TC-968: delivery rejects dangling references (scenario) — passing

### FT-127 — product deliverable/release done — the §7.2 computed delivery predicates [x]

- [x] ADR-068: Done is computed from existing verifications plus recorded acceptance (accepted)
- [x] TC-969: deliverable done lifecycle (scenario) — passing
- [x] TC-975: release done requires members done and closed cut (scenario) — passing

### FT-128 — CLI↔MCP parity for the decider, slice, deliverable, and release families [x]

- [x] ADR-069: The framework families expose MCP tools mirroring their CLI subcommands (accepted)
- [x] TC-976: decider MCP parity via call_tool (scenario) — passing
- [x] TC-977: delivery MCP parity via call_tool (scenario) — passing

### FT-129 — CLI↔MCP parity for archetype, cell, how, work-unit, and dep — PENDING_MCP cleared [x]

- [x] ADR-070: The remaining families expose read MCP tools; PENDING_MCP is emptied (accepted)
- [x] TC-978: framework read MCP parity via call_tool (scenario) — passing
- [x] TC-979: dependency MCP parity via call_tool (scenario) — passing

### FT-130 — product build — the SPMC build orchestrator that records conformance into done [x]

- [x] ADR-071: build assembles the SPMC context; conformance is recorded into done (accepted)
- [x] TC-983: recording conformance flips done (scenario) — passing
- [x] TC-984: build dry-run assembles the SPMC context (scenario) — passing

### FT-131 — worker capability catalog — role to capability with escalation, claude or litellm [x]

- [x] ADR-072: Workers are capabilities resolved by role with escalation; runners are claude or litellm (accepted)
- [x] ADR-076: The build gates forbid a worker from editing the acceptance tests (oracle integrity) (accepted)
- [x] TC-985: worker resolve escalates by trigger (scenario) — passing
- [x] TC-986: build resolves worker by role (scenario) — passing
- [x] TC-989: worker check flags an unknown endpoint (scenario) — passing

### FT-132 — parallel work-unit execution — build fans units across workers [x]

- [x] ADR-073: Work units are the parallel unit, fanned out bounded with a coherence gate (accepted)
- [x] ADR-075: Work units dispatch in dependency order, with upstream artifacts injected as frozen inputs (accepted)
- [x] ADR-077: The work-unit path is mandatory and harness-authoritative; cells template it per slot (accepted)
- [x] TC-987: build parallel plan lists work units (scenario) — passing

### FT-133 — first-party worker — a native SPMC executor (endpoint worker) [x]

- [x] ADR-074: The first-party worker is a structured-output capability endpoint (accepted)
- [x] TC-988: first-party worker writes artifact offline (scenario) — passing
...
- [x] ADR-080: Read models declare a state space; UI steps cover it constrained and complete (accepted)
- [ ] TC-1000: UI step covers every projection state (scenario) — unimplemented
- [ ] TC-1001: forgotten failed state fails coverage (scenario) — unimplemented
- [ ] TC-1002: waiving an ignorable state passes with reason (scenario) — unimplemented
...
- [x] ADR-083: Screens bind to a design system; AIOs reify to CIOs by context of use (accepted)
- [ ] TC-1009: AIO reifies to different CIOs by context (scenario) — unimplemented
- [ ] TC-1010: reification coverage over AIO and context (scenario) — unimplemented
- [ ] TC-1011: off-system component and literal style are rejected (scenario) — unimplemented
