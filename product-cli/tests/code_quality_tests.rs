...
// =============================================================================
// FT-118 / TC-980: cli_mcp_parity — every CLI command family must either expose
// the same functionality as an MCP tool (product_<cmd>_*) or be explicitly
// classified. This enforces the recurring rule "when you add a command, also
// expose it as an MCP tool" — a new command in neither bucket fails the gate.
// =============================================================================
...
const PENDING_MCP: &[&str] = &[
    "archetype", "cell", "decider", "deliverable", "dep", "how", "release", "slice", "work-unit",
];
...
fn mcp_tool_names() -> Vec<String> {
    ...
    while let Some(start) = rest.find("\"product_") {
        rest = &rest[start + 1..];
        if let Some(end) = rest.find('"') {
            tools.push(rest[..end].to_string());
            rest = &rest[end + 1..];
        }
    }
    tools
...
fn command_names() -> Vec<String> {
    let src = std::fs::read_to_string(project_root().join("product-cli/src/commands/root_enum.rs"))
        .expect("read root_enum.rs");
    parse_subcommand_enums(&src)
        .into_iter()
...
fn has_mcp_tool(cmd: &str, tools: &[String]) -> bool {
    let prefix = format!("product_{}", cmd.replace('-', "_"));
    tools.iter().any(|t| t.starts_with(&prefix))
}

#[test]
fn tc_980_cli_commands_have_mcp_parity() {
    let commands = command_names();
    let tools = mcp_tool_names();
    assert!(commands.len() >= 30, "expected the full command set, got {}", commands.len());

    // Every command must be MCP-exposed, CLI-only, or documented debt.
    let mut unclassified = Vec::new();
    for c in &commands {
        let ok = has_mcp_tool(c, &tools) || CLI_ONLY.contains(&c.as_str()) || PENDING_MCP.contains(&c.as_str());
        if !ok {
    ...
    assert!(
        ...
        "cli-mcp-parity: these commands have no MCP tool and are not classified — \
         add an MCP tool (product_<cmd>_*) or list them in CLI_ONLY / PENDING_MCP:\n  {}",
        unclassified.join("\n  "),
    );

    // PENDING_MCP is self-cleaning: an entry that now HAS a tool must be removed.
    for p in PENDING_MCP {
        assert!(
            !has_mcp_tool(p, &tools),
            "'{p}' now has an MCP tool — remove it from PENDING_MCP",
        );
    }
    // No stale classification entries.
    for x in CLI_ONLY.iter().chain(PENDING_MCP.iter()) {
        assert!(commands.contains(&x.to_string()), "stale classification: '{x}' is not a command");
    }
}
