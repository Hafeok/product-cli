...
fn cell_init_scaffolds_a_task_type() {
    ...
    let out = handle_cell_init(&json!({"id": "add-crud-resource", "blueprint": "rest-api"}), root).expect("init");
    assert_eq!(out["ok"], json!(true));
    assert_eq!(out["blueprint"], json!("rest-api"));
...
fn cell_dispatch_reports_unbound_required_slots() {
    let r = repo();
    let root = r.path();
    handle_cell_init(&json!({"id": "add-crud-resource", "blueprint": "rest-api"}), root).expect("init");
...
#[test]
fn blueprint_init_lays_down_the_skeleton() {
    let r = repo();
    let root = r.path();

    let out = handle_blueprint_init(&json!({"name": "rest-api"}), root).expect("init");
    assert_eq!(out["ok"], json!(true));
    assert_eq!(out["written"].as_array().map(|a| a.len()), Some(3));

    let dir = pdir(root).join("blueprints").join("rest-api");
    ...
    // The read handler now sees the new blueprint.
    let listed = crate::framework_read_handlers::handle_blueprint_list(&json!({}), root).expect("list");
    assert!(listed["blueprints"].as_array().map(|a| a.iter().any(|v| v == "rest-api")).unwrap_or(false));

    // Re-init without force is refused.
    assert!(handle_blueprint_init(&json!({"name": "rest-api"}), root).is_err());
}
