...
fn blueprints_dir(base: &Path) -> PathBuf {
    let canonical = base.join("blueprints");
    if canonical.exists() {
        return canonical;
    }
    // Back-compat: fall back to the legacy `<base>/archetypes/` if present.
...
pub fn handle_blueprint_list(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let mut names: Vec<String> = match std::fs::read_dir(blueprints_dir(&pbase(args, repo_root))) {
...
fn load_blueprint(base: &Path, name: &str) -> Result<Blueprint, String> {
    Blueprint::load_from_dir(&blueprints_dir(base).join(name), name).map_err(|e| format!("{e}"))
}

pub fn handle_blueprint_show(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let name = req_str(args, "name")?;
    let a = load_blueprint(&pbase(args, repo_root), &name)?;
    Ok(json!({
        "name": name,
        "how": a.how.is_some(),
...
pub fn handle_blueprint_validate(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let a = load_blueprint(&pbase(args, repo_root), &req_str(args, "name")?)?;
...
fn load_cell(base: &Path) -> Result<TaskType, String> {
    let path = base.join("cell.yaml");
    let text = std::fs::read_to_string(&path).map_err(|_| format!("no cell at {}", path.display()))?;
    TaskType::from_yaml(&text).map_err(|e| format!("{e}"))
}

pub fn handle_cell_show(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let c = load_cell(&pbase(args, repo_root))?;
...
pub fn handle_cell_validate(args: &Value, repo_root: &Path) -> Result<Value, String> {
    ...
    let c = load_cell(&base)?;
    let domain = graph_of(args, repo_root).ok();
    let how = load_yaml(&base, "how-contract", HowContract::from_yaml).ok();
...
pub fn handle_how_show(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let h = load_how(&pbase(args, repo_root))?;
    Ok(json!({
...
pub fn handle_how_validate(args: &Value, repo_root: &Path) -> Result<Value, String> {
    Ok(verdict(&validate_how(&load_how(&pbase(args, repo_root))?)))
}

pub fn handle_how_export(args: &Value, repo_root: &Path) -> Result<Value, String> {
    Ok(json!({ "turtle": how_to_turtle(&load_how(&pbase(args, repo_root))?) }))
}

// --- work-unit -------------------------------------------------------------

fn load_work_unit(base: &Path) -> Result<WorkUnit, String> {
    load_yaml(base, "work-unit", WorkUnit::from_yaml)
}

pub fn handle_work_unit_show(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let w = load_work_unit(&pbase(args, repo_root))?;
...
pub fn handle_work_unit_validate(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let base = pbase(args, repo_root);
    let w = load_work_unit(&base)?;
    let domain = graph_of(args, repo_root).ok();
    let how = load_how(&base).ok();
    Ok(verdict(&validate_work_unit(&w, domain.as_ref(), how.as_ref())))
}

// --- worker (capability catalog) -------------------------------------------

fn load_catalog(base: &Path) -> Catalog {
    let p = base;
...
pub fn handle_worker_list(args: &Value, repo_root: &Path) -> Result<Value, String> {
    let c = load_catalog(&pbase(args, repo_root));
