...
fn projectors_dir(base: &Path) -> std::path::PathBuf {
    base.join("projectors")
}

fn load(base: &Path, name: &str) -> Result<Projector, String> {
    load_yaml(&projectors_dir(base), name, Projector::from_yaml)
}

pub fn handle_projector_list(args: &Value, repo_root: &Path) -> Result<Value, String> {
    Ok(json!({ "projectors": ids_in(&projectors_dir(&pbase(args, repo_root))) }))
}
...
pub fn handle_projector_derive(args: &Value, repo_root: &Path) -> Result<Value, String> {
    ...
    let dir = projectors_dir(&pbase(args, repo_root));
