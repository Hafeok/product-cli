//! Codegen tool definitions — verifiable code projections over MCP.
//!
//! Four tools, gated to the Build phase by the `product_codegen_` prefix:
...
fn backends_tool() -> ToolDef {
    ToolDef {
        name: "product_codegen_backends".to_string(),
        description: "List the built-in codegen language backends (id, description, tier). External backends run out-of-band via `product codegen plugin` — a process consuming the codegen manifest.".to_string(),
...
fn check_tool() -> ToolDef {
    ToolDef {
        name: "product_codegen_check".to_string(),
...
fn emit_tool() -> ToolDef {
    ToolDef {
        name: "product_codegen_emit".to_string(),
