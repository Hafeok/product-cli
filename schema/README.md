...
schema/
  ...
  shapes/
    shapes.shacl.ttl               # SHACL — validates that a product graph conforms (the conformance checker)
  json/
...
| Top decision, principle, pattern | §4.1 The Why | `ontology` + `shapes` | RDF/SHACL |
| Application & infrastructure contracts | §4.2 | `ontology` + `shapes` | RDF/SHACL |
| **Repository layout model** | §4.3 | `layout-model.schema.json` | YAML/JSON |
| Interface contract (OpenAPI/AsyncAPI/…) | §4.4 | *use the standard's own schema* | — |
...
A malformed graph (an orphan event, a why-less decision) fails with those messages.
