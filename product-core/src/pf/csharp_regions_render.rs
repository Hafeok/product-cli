...
fn render_ratios(r: &RegionsReport) -> String {
    let x = &r.ratios;
    let mut s = format!(
        "\nsplit over undeclared production types (walk from {} roots of {} accepted entry points, hosts' sites, O-17 off):\n  reachable-undeclared:  {} of {} ({:.1}%)\n  unresolved-undeclared: {} of {}\n  isolated-undeclared:   {} of {} ({:.1}%)\n  error bound (CG-R-120, in force): {} unfollowed of {} composition edges = {:.1}%\n      composition: unresolved {} · registration-not-read {} (the table deciding the latter is unvalidated, CG-R-101)\n  prior form (CG-R-89, beside it): {} unscored of {} = {:.1}%\n",
        x.roots, x.accepted_entry_points, x.reachable_undeclared, x.undeclared_types, x.reachable_percent, x.unresolved_undeclared, x.undeclared_types, x.isolated_undeclared, x.undeclared_types, x.isolated_percent, x.unfollowed, x.composition_edges, x.error_bound_percent, x.unresolved_edges, x.not_read_edges, x.unscored, x.composition_edges, x.unscored_fraction_percent
    );
