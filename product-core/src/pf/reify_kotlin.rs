...
const BUILD_GRADLE: &str = r##"// Scaffolded once by `product reify kotlin` (never overwritten):
...
const CONFORMANCE_STUB_KT: &str = r##"// Scaffolded once by `product reify kotlin` — never overwritten.
...
const PROJECTION_STUB_KT: &str = r##"// Scaffolded once by `product reify kotlin` — never overwritten.
...
const SCREEN_STUB_KT: &str = r##"// Scaffolded once by `product reify kotlin` — never overwritten.
...
Generated by `product reify kotlin` (oracle-only): no domain types are
...
3. `product reify check --out <this directory>` — drift gate on the graph hash.
"##;
...
pub fn plan_kotlin(
    ...
    opts: &ReifyOptions,
) -> crate::error::Result<super::reify::ReifyPlan> {
    use super::reify::{aggregate_names, gen as rgen, input_hash, header, provenance_json, GenFile as GF, ReifyPlan};
    ...
    files.push(rgen(
        ...
        super::reify_cell::cell_file_kotlin(&graph_hash, &pkg)?,
    ));
    files.push(GF {
...
fn fact_files(
    ...
    use super::reify_kotlin_facts as kf;
    ...
    for p in sorted_p {
        if !p.scenarios.is_empty() {
            let base = super::reify_projector::view_base(p);
            out.push(gen(&format!("{test_dir}/{base}ProjectionTests.g.kt"), kf::projector_tests(hdr, pkg, p)));
        }
    }
    let flows = super::reify_flow::plan_flows(graph, sorted, sorted_p, true);
    if !flows.is_empty() {
        out.push(gen(&format!("{test_dir}/FlowTests.g.kt"), kf::flow_tests(hdr, pkg, &flows)));
    }
    for step in steps {
        out.push(gen(
            &format!("{test_dir}/{}ScreenTests.g.kt", super::reify_ident::pascal(&step.id)),
