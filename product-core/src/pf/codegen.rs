//! Codegen orchestration — plan a verifiable C# projection of the What graph.
//!
...
//! (canonical Turtle + decider YAMLs) so `codegen check` is a drift gate:
...
use super::codegen_decider::{adapter_file, frame_file, stub_file};
use super::codegen_ident::pascal;
use super::codegen_infer::{infer_shape, AggShape};
use super::codegen_program::{program_file, RunnerEntry};
use super::codegen_runtime as runtime;
use super::codegen_scenarios::tests_file;
use super::codegen_types::{agg_types_file, domain_file};

pub use super::reify_ds::recorded_ds;
pub use super::codegen_prov::{provenance_json, recorded_hash};
...
pub struct ReifyOptions {
    ...
    /// `.product/design-systems/<id>/`. When present, codegen gates on the
...
/// The full deterministic file set for one `codegen csharp` run.
...
/// projector YAML (sorted by id) — the identity `codegen check` compares
/// against.
pub fn input_hash(
    graph: &DomainGraph,
    product: &str,
...
pub fn header(graph_hash: &str, what_version: &str) -> String {
    format!(
        "// <auto-generated/>\n// Generated by `product codegen` — do not edit; regenerate instead.\n// pf:graph-hash: sha256:{graph_hash}\n// pf:what-version: {what_version}\n\n"
    )
}
...
pub fn plan_csharp(
    ...
    for (decider, agg) in sorted.iter().zip(&aggs) {
        let shape = infer_shape(decider, graph);
    ...
    files.push(gen(
        ...
        super::codegen_cell::cell_file(&graph_hash, &opts.namespace, opts.oracle_only)?,
    ));
    files.push(GenFile {
...
fn projector_files(opts: &ReifyOptions, hdr: &str, projector: &Projector) -> Vec<GenFile> {
    use super::codegen_projector as rp;
...
pub(crate) fn aggregate_names(sorted: &[&Decider]) -> Result<Vec<String>> {
...
fn oracle_global_files(opts: &ReifyOptions, hdr: &str, hash: &str, has_projectors: bool) -> Vec<GenFile> {
    ...
    let mut out = vec![
        ...
        GenFile {
            ...
            content: super::codegen_oracle::oracle_csproj(),
            overwrite: false,
        },
        GenFile {
            path: format!("{conf}/ConformanceAdapter.cs"),
            content: super::codegen_oracle::adapter_stub(ns),
            overwrite: false,
        },
        gen(
    ...
    if has_projectors {
        out.push(GenFile {
            ...
            content: super::codegen_oracle::projection_stub(ns),
            overwrite: false,
        });
    }
    out
...
fn oracle_aggregate_files(opts: &ReifyOptions, hdr: &str, decider: &Decider, agg: &str) -> Vec<GenFile> {
    ...
    vec![gen(
        ...
        super::codegen_oracle::wire_tests_file(hdr, ns, agg, decider),
    )]
}
...
fn integration_files(
    ...
    let mut out = Vec::new();
    let flows = super::codegen_flow::plan_flows(graph, sorted, sorted_p, opts.oracle_only);
    if !flows.is_empty() {
        out.push(gen(&format!("{tests}/FlowTests.g.cs"), super::codegen_flow::tests_file(hdr, ns, &flows)));
    }
    ...
    if !steps.is_empty() {
        ...
        out.push(gen(&format!("{oracle_home}/ScreenSeam.g.cs"), super::codegen_screen::seam_file(hdr, ns)));
        out.push(GenFile {
            path: format!("{ns}.Conformance/ScreenAdapter.cs"),
            content: super::codegen_screen::adapter_stub(ns),
            overwrite: false,
        });
        for step in &steps {
            out.push(gen(
                &format!("{tests}/{}ScreenTests.g.cs", pascal(&step.id)),
                super::codegen_screen::tests_file(hdr, ns, step, sorted_p),
            ));
        }
    }
...
fn conformance_files(
    ...
    let projections: Vec<RunnerEntry> = sorted_p
        ...
        .map(|p| RunnerEntry {
            ...
            agg: super::codegen_projector::view_base(p),
        })
        .collect();
    let conf = format!("{ns}.Conformance");
    ...
    let mut out = vec![
        gen(&format!("{oracle_home}/Oracle.g.cs"), super::codegen_oracle::oracle_file(hdr, ns)),
        gen(
