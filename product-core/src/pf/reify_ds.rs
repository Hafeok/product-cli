...
use super::codegen::{gen, GenFile, ReifyOptions};
...
pub fn resolve(spec: &DsSpec, graph: &DomainGraph) -> Result<ResolvedDs> {
    ...
    for step in super::codegen_screen::testable_steps(graph) {
...
pub fn tokens_css(spec: &DsSpec) -> String {
    ...
    let mut s = format!(
        "/* generated by `product codegen` — design system '{}' v{} — tokens, not literals (§4.5) */\n",
        ds.id, ds.version
    );
...
fn cs(s: &str) -> String {
    super::codegen_ident::cs_escape(s)
}
