//! Unit tests for the §12.2 content-store manifest profile.
...
fn missing_locale_value_fails() {
    let src = WHOLE.replace(", de = \"Kasse\"", "");
    let m = parse_content(&src).expect("parses");
...
fn empty_error_role_fails() {
    let src = WHOLE.replace("Your cart is empty", "");
    let m = parse_content(&src).expect("parses");
    let f = validate_content(&m);
...
fn coupling_resolves_referenced_keys_per_locale() {
    ...
    // Reference an unresolved key → non-conforming, naming (key, locale).
