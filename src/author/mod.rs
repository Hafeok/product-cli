...
pub fn review_staged(root: &Path) -> Result<Vec<String>> {
    ...
    for adr_path in &staged_adrs {
        review_single_adr(root, adr_path, &mut findings);
    }
...
fn review_single_adr(root: &Path, adr_path: &str, findings: &mut Vec<String>) {
    ...
    for (name, marker) in &required_sections {
        if !content.contains(marker) && !content.to_lowercase().contains(&name.to_lowercase()) {
            findings.push(format!("warning: {} missing required section: {}", adr_path, name));
        }
    }

    if !content.contains("status:") {
        findings.push(format!("warning: {} missing status field in front-matter", adr_path));
    }

    if content.contains("features: []") || !content.contains("features:") {
        findings.push(format!("warning: {} has no linked features", adr_path));
    }
}
...
fn build_commit_message(session_type: &SessionType, changed_files: &[String]) -> String {
    ...
    let id_summary = if ids.len() <= 5 { ids.join(", ") } else { format!("{} artifacts", ids.len()) };
    format!("author({}): {}\n\nAuto-committed by product author session.", session_type, id_summary)
}

fn run_git_commit(root: &Path, message: &str) {
    let add = Command::new("git").args(["add", "docs/"]).current_dir(root).status();
    if !matches!(add, Ok(s) if s.success()) {
        eprintln!("Failed to stage changes. Commit manually.");
        return;
    }

    let commit = Command::new("git").args(["commit", "-m", message]).current_dir(root).status();
    match commit {
        Ok(s) if s.success() => {
...
fn schema_prompt() -> String {
    let mut out = String::from("# Artifact Schemas\n\nAll artifacts use YAML front-matter between `---` delimiters, followed by a markdown body.\n\n");
    out.push_str(&schema_feature_section());
    out.push_str(&schema_adr_section());
    out.push_str(&schema_tc_section());
    out
}

fn schema_feature_section() -> String {
    r#"## Feature (FT-XXX)
...
fn schema_tc_section() -> String {
    r#"## Test Criterion (TC-XXX)
